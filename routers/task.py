...
def _estimate_duration(required_cpu: int, required_ram: int) -> float:
    """Estimate task duration based on CPU and RAM requirements."""
...
def _claim_next_task_with_rl(
    worker_id: str,
    now: datetime,
    available_cpu: int,
    available_ram: int,
) -> dict | None:
    """Claim the best task using RL scoring."""
    agent = _get_agent()
    ...
    # Score each candidate using RL weights
    scored_tasks = []
    for task in candidates:
        # Get RL state and weights for this task-worker pair
        rl_state = agent.discretize_state(
            worker_cpu=available_cpu,
            worker_ram=available_ram,
            task_cpu=task.get("required_cpu", 0),
            task_ram=task.get("required_ram", 0),
        )
        w_priority, w_duration = agent.get_weights(rl_state)
        ...
        # Normalize priority (assumed 0-5 scale)
        normalized_priority = task.get("priority", 0) / 5.0
        duration_score = 1 / max(estimated_duration, 0.1)

        # Calculate RL-informed score
        score = (w_priority * normalized_priority) + (w_duration * duration_score)
    ...
    # Try to claim the best task atomically
    for scored in scored_tasks:
        task = scored["task"]
        rl_state = scored["rl_state"]
        rl_action = agent.get_action(rl_state)

        claimed = task_collection.find_one_and_update(
            {
                "_id": task["_id"],
                "status": "pending",
            },
...
def _claim_next_task_fallback(
    worker_id: str,
    now: datetime,
    available_cpu: int,
    available_ram: int,
) -> dict | None:
    """Fallback: simple priority-based claim (no RL) with correct allocation values."""
    # Step 1: Find the best candidate task
    ...
    # Step 2: Atomically claim it with correct numeric values
...
def _assign_tasks_non_transactional(
    ...
    while (
        ...
        for attempt in range(1, MAX_ASSIGNMENT_RETRIES + 1):
            # Try RL-based claim first
            task = _claim_next_task_with_rl(
                worker_id=worker_id,
                now=now,
            ...
            # Fallback to simple priority if RL fails
            if not task:
                task = _claim_next_task_fallback(
                    worker_id=worker_id,
                    now=now,
                    available_cpu=available_cpu,
                    available_ram=available_ram,
